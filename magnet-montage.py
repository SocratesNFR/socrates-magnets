...
def gen_labels(input_files, fmt="{}"):
    inds = range(len(input_files))
    return map(fmt.format, inds)

def montage_single(output_file, input_files, labels=True, label_format="{}"):
    if labels:
        labels = gen_labels(input_files, label_format)

    print("montage {}".format(output_file))

    montage(output_file, input_files, labels)

def montage_dirs(output_dir, input_dirs, labels=True, label_format="{}", glob_pattern='*.png'):
    ...
    for img in imgs:
        ...
        montage_single(output_file, input_files, labels, label_format)
...
def main(args):
    ...
    if all(input_dirs):
        ...
        montage_dirs(args.output, args.files, args.labels, args.label_format)
        ...
        # Single output image
        montage_single(args.output, args.files, args.labels, args.label_format)
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-l', '--labels', action='store_true', default=False,
            help='enable labels')
    parser.add_argument('-f', '--label-format', default='{}',
            help='label format string')
